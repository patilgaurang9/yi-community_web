...
import type { MemberData, EventData, OfferData } from "@/types/buzz"
import { BuzzImage } from "./buzz-image"
...
function OfferCard({ offer }: { offer: OfferData }) {
  ...
  const handleCopy = (e: React.MouseEvent) => {
    ...
    if (offer.description) {
      navigator.clipboard.writeText(offer.description)
      setCopied(true)
    ...
    <div className="bg-zinc-900 border-2 border-dashed border-zinc-700 rounded-lg p-4">
      ...
      {offer.description && (
        <div className="flex items-center gap-2">
          <p className="flex-1 text-xs text-zinc-400">{offer.description}</p>
          <Button
            variant="outline"
            size="sm"
            onClick={handleCopy}
